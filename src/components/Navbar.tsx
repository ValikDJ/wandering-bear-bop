...
import { sidebarNavData, SidebarNavItem } from "@/data/sidebarNavData"; // Імпорт даних бічної панелі для мобільного меню
...
const Navbar: React.FC<NavbarProps> = ({ onOpenMobileSidebar }) => {
  ...
  const renderMobileNavLinks = (items: SidebarNavItem[], level: number = 0) => {
    return items.map(item => {
      ...
      const indentClass = level === 1 ? "pl-8" : level === 2 ? "pl-12" : "pl-4"; // Налаштування відступу

      if (isGroup) {
        return (
          <React.Fragment key={item.id}>
            {/* Заголовок групи - не клікабельний як пряме посилання, а як заголовок */}
        ...
        // Фактичне навігаційне посилання
        return (
      ...
      <div className="container mx-auto flex flex-wrap justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          {isMobile ? (
            ...
            // Навігаційні посилання для десктопу - спрощені, щоб більше покладатися на бічну панель
